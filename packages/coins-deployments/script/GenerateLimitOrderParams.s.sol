...
contract GenerateLimitOrderParams is CoinsDeployerBase {
    ...
    function mineForLimitOrderBook(
        ...
        bytes memory creationCode = abi.encodePacked(
            type(LeakLimitOrderBook).creationCode,
            abi.encode(poolManager, leakFactory, leakHookRegistry, owner, weth)
        );
        ...
        config.constructorArgs = abi.encode(poolManager, leakFactory, leakHookRegistry, owner, weth);
        config.contractName = "LeakLimitOrderBook";
    }
    ...
    function mineForSwapRouter(
        ...
        config.deployedAddress = expectedAddress;
        config.creationCode = creationCode;
        config.constructorArgs = abi.encode(poolManager, leakLimitOrderBook, swapRouter, permit2, owner);
    ...
    function run() public {
        ...
        saveDeterministicContractConfig(limitOrderBookConfig, "leakLimitOrderBook");
        saveDeterministicContractConfig(swapRouterConfig, "zoraRouter");

        // Save to deployment file
        deployment.leakLimitOrderBook = limitOrderBookConfig.deployedAddress;
        ...
        console2.log("LEAK_LIMIT_ORDER_BOOK:", limitOrderBookConfig.deployedAddress);
        console2.log("SWAP_ROUTER_SALT:", vm.toString(swapRouterConfig.salt));
        console2.log("LEAK_ROUTER:", swapRouterConfig.deployedAddress);
        console2.log("\nConfiguration files saved to deterministicConfig/");
        console2.log("  - leakLimitOrderBook.json");
