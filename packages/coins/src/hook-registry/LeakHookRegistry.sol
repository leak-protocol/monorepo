...
/// @title Leak Hook Registry
/// @notice A registry of Leak hook contracts for Uniswap V4
contract LeakHookRegistry is ILeakHookRegistry, MultiOwnable {
    ...
    function getHooks() external view returns (LeakHook[] memory) {
        uint256 numHooks = registeredHooks.length();

        LeakHook[] memory hooks = new LeakHook[](numHooks);

        for (uint256 i; i < numHooks; i++) {
            address hook = registeredHooks.at(i);

            hooks[i] = LeakHook({hook: hook, tag: getHookTag(hook), version: getHookVersion(hook)});
        }
    ...
    function registerHooks(address[] calldata hooks, string[] calldata tags) external onlyOwner {
        ...
        for (uint256 i; i < hooks.length; i++) {
            if (registeredHooks.add(hooks[i])) {
                ...
                emit LeakHookRegistered(hooks[i], tags[i], getHookVersion(hooks[i]));
            }
        }
    }
    ...
    function removeHooks(address[] calldata hooks) external onlyOwner {
        for (uint256 i; i < hooks.length; i++) {
            if (registeredHooks.remove(hooks[i])) {
                emit LeakHookRemoved(hooks[i], hookTags[hooks[i]], getHookVersion(hooks[i]));
