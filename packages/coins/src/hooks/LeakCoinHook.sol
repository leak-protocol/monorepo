...
import {ILeakCoinHook} from "../interfaces/ILeakCoinHook.sol";
...
import {ILeakHookRegistry} from "../interfaces/ILeakHookRegistry.sol";
...
/// @author oveddan
contract LeakCoinHook is BaseHook, ContractVersionBase, ILeakCoinHook, ERC165, IUpgradeableDestinationV4Hook, IUpgradeableDestinationV4HookWithUpdateableFee {
    ...
    mapping(bytes32 => ILeakCoinHook.PoolCoin) internal poolCoins;
    ...
    ILeakLimitOrderBook internal immutable leakLimitOrderBook;

    /// @notice The Leak hook registry
    ILeakHookRegistry internal immutable leakHookRegistry;

    /// @notice The constructor for the LeakCoinHook.
    ...
    /// @param leakLimitOrderBook_ The Zora limit order book contract for filling orders during swaps
    /// @param leakHookRegistry_ The Leak hook registry contract for identifying registered hooks
    constructor(
        ...
        ILeakLimitOrderBook leakLimitOrderBook_,
        ILeakHookRegistry leakHookRegistry_
        ...
        require(address(leakLimitOrderBook_) != address(0), LeakLimitOrderBookCannotBeZeroAddress());
        require(address(leakHookRegistry_) != address(0), LeakHookRegistryCannotBeZeroAddress());
        ...
        leakLimitOrderBook = leakLimitOrderBook_;
        leakHookRegistry = leakHookRegistry_;
    }
    ...
    /// @inheritdoc ILeakCoinHook
    function getPoolCoinByHash(bytes32 poolKeyHash) external view returns (ILeakCoinHook.PoolCoin memory) {
        return poolCoins[poolKeyHash];
    }

    /// @inheritdoc ILeakCoinHook
    function getPoolCoin(PoolKey memory key) external view returns (ILeakCoinHook.PoolCoin memory) {
    ...
    function _afterSwap(
        ...
        if (tickAfterSwap != tickBeforeSwap) {
            ...
            leakLimitOrderBook.fill(key, isCurrency0, tickBeforeSwap, tickAfterSwap, CoinConstants.SENTINEL_DEFAULT_LIMIT_ORDER_FILL_COUNT, address(0));
        }
    ...
    function _isInternalSwap(address sender) internal view returns (bool) {
        return sender == address(this) || sender == address(leakLimitOrderBook) || leakHookRegistry.isRegisteredHook(sender);
    }
