...
interface ILeakHookRegistry {
    ...
    event LeakHookRegistered(address indexed hook, string tag, string version);

    /// @notice Emitted when a hook is removed from the registry
    event LeakHookRemoved(address indexed hook, string tag, string version);
