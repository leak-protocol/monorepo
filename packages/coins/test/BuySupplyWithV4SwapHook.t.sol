...
contract BuySupplyWithV4SwapHookTest is BaseTest {
    address constant BACKING = WETH_ADDRESS  /* leak.ai: WAVAX */;
    ...
    function _deployCreatorCoin(address payoutRecipient) internal returns (address creatorCoinAddress) {
        bytes memory creatorPoolConfig = CoinConfigurationVersions.defaultDopplerMultiCurveUniV4(BACKING);
        ...
        creatorCoinAddress = factory.deployCreatorCoin(
            ...
            "Creator Coin", // name
            "CREATOR", // symbol
            creatorPoolConfig, // poolConfig (BACKING-backed)
    ...
    /// @dev Test buying initial supply of a Content Coin backed by BACKING
    /// This only requires V3 swap (ETH -> BACKING) since the coin is already backed by BACKING
    function test_buyContentCoinSupply_V3SwapOnly() public {
        ...
        // Create V3 path: ETH -> USDC -> BACKING
        bytes memory v3Route = _encodeV3Path(
            address(weth),
            3000, // WETH/USDC 0.3%
            USDC_ADDRESS,
            3000, // USDC/BACKING 0.3%
            BACKING
        );

        console.logBytes(v3Route);

        // No V4 route needed since coin is backed by BACKING
        ...
        // Deploy Content Coin backed by BACKING
        (address coinAddress, uint256 amountCurrency, uint256 coinsPurchased) = _deployContentCoinWithHook(BACKING, initialOrderSize, users.creator, hookData);

        ContentCoin coin = ContentCoin(payable(coinAddress));

        // Verify the coin is properly configured
        assertEq(coin.currency(), BACKING, "Coin should be backed by BACKING");
        assertGt(amountCurrency, 0, "Should have received BACKING from V3 swap");
    ...
    function test_BuyInitialSupplyEvent() public {
        ...
        // Deploy Content Coin backed by BACKING - this should emit event with matching parameters
        _deployContentCoinWithHook(BACKING, initialOrderSize, users.creator, hookData);
    }
    ...
    function test_buyContentCoinSupply_WithOwnedCreatorCoins() public {
        // STEP 1: Deploy Creator Coin backed by BACKING
        address creatorCoinAddress = _deployCreatorCoin(users.creator);

        // STEP 2: Give another user BACKING tokens and have them swap for Creator Coins
        address anotherCreator = makeAddr("anotherCreator");
        uint256 backingAmount = 10e18; // 10 BACKING tokens
        deal(BACKING, anotherCreator, backingAmount);
        assertEq(IERC20(BACKING).balanceOf(anotherCreator), backingAmount, "anotherCreator should have BACKING tokens");

        // Swap BACKING tokens for Creator Coins using proper V4 swap mechanism
        uint128 swapAmountIn = uint128(backingAmount);
        _swapSomeCurrencyForCoin(ICoin(payable(creatorCoinAddress)), BACKING, swapAmountIn, anotherCreator);
    ...
    /// This requires V3 swap (ETH -> BACKING) then V4 swap (BACKING -> Creator Coin -> Content Coin)
    function test_buyContentCoinSupply_CreatorCoinBacked() public {
        ...
        // Create V3 path: ETH -> USDC -> BACKING (to get the creator coin's backing currency)
        bytes memory v3Route = _encodeV3Path(
            address(weth),
            3000, // WETH/USDC 0.3%
            USDC_ADDRESS,
        ...
        // V4 route: BACKING -> Creator Coin (then Creator Coin -> Content Coin will be added automatically)
    ...
    function test_RevertWhen_InsufficientInputCurrencyETH() public {
        ...
        bytes memory poolConfig = CoinConfigurationVersions.defaultDopplerMultiCurveUniV4(BACKING);
    ...
    function test_RevertWhen_InsufficientInputCurrencyERC20() public {
        ...
        uint256 backingAmount = 10e18; // 10 BACKING tokens
        deal(BACKING, users.creator, backingAmount);

        // Swap BACKING tokens for Creator Coins using proper V4 swap mechanism
        uint128 swapAmountIn = uint128(backingAmount);
        _swapSomeCurrencyForCoin(ICoin(payable(creatorCoinAddress)), BACKING, swapAmountIn, users.creator);
    ...
    function test_RevertWhen_V3RouteDoesNotConnectToV4RouteStart() public {
        // Deploy Creator Coin backed by BACKING
        address creatorCoinAddress = _deployCreatorCoin(users.creator);
        ...
        // Create V4 route that starts with BACKING (not USDC - mismatch!)
        PoolKey[] memory v4Route = new PoolKey[](1);
        v4Route[0] = ICoin(payable(creatorCoinAddress)).getPoolKey();
