...
contract CoinUniV4Test is BaseTest {
    ...
    function test_swap_emitsCoinMarketRewardsV4() public {
        ...
        emit ILeakCoinHook.CoinMarketRewardsV4(
            address(coinV4),
            address(mockERC20A),
    ...
    function test_afterInitializeRevertsWhenSenderIsNotACoin() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ...
                abi.encodeWithSelector(ILeakCoinHook.NotACoin.selector, address(notACoin)),
    ...
    function test_getSwapPath_whenBackingCurrencyProvidesPath() public {
        address backing = address(mockERC20A);
        ICoin backingCoin = _deployV4Coin(backing);
        ...
        _assertPathKeyEqual(
            path[1],
            PathKey({
                intermediateCurrency: Currency.wrap(backing),
                fee: backingCoin.getPoolKey().fee,
                tickSpacing: backingCoin.getPoolKey().tickSpacing,
                hooks: backingCoin.getPoolKey().hooks,
                hookData: bytes("")
            }),
            "backing to backing coin"
        );
    }

    function test_swap_withBackingCoinToBacking_paysRewardsInBackingOnly(uint128 amountIn) public {
        // backing is a mock erc20
        address backing = address(mockERC20A);
        // make sure pool manager has enough backing in it so we can take the fees on the swap
        mockERC20A.mint(address(poolManager), 10000000000000000 ether);

        // backing coin is a mock coin that is paired with backing
        ICoin backingCoin = _deployV4Coin(backing);
        // now create a final coin paired with the backing coin
        ICoin contentCoin = _deployV4Coin(address(backingCoin));
        ...
        address trader = makeAddr("trader");
        MockERC20(backing).mint(trader, amountIn);

        address protocolRewardRecipient = contentCoin.protocolRewardRecipient();

        // swap some backing for backing coin, so the trader has some backing coin - this should not cause a multihop swap for rewards
        _swapSomeCurrencyForCoin(backingCoin, backing, uint128(IERC20(address(backing)).balanceOf(trader)), trader);

        // get balances before
        uint256 protocolRewardRecipientBackingBalanceBefore = IERC20(address(backing)).balanceOf(protocolRewardRecipient);
        ...
        // make sure that no backing, backing, or content coin was paid out to the protocol reward recipient, but just usdc was
        ...
        assertGt(IERC20(address(backing)).balanceOf(protocolRewardRecipient), protocolRewardRecipientBackingBalanceBefore, "backing was paid out");
    }
