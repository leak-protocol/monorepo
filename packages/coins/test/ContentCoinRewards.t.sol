...
contract ContentCoinRewardsTest is BaseTest {
    ...
    function _calculateBackingRewardDeltas(RewardBalances memory initialBalances) internal view returns (RewardBalances memory deltas) {
        deltas.creator = backingToken.balanceOf(users.creator) - initialBalances.creator;
        deltas.platformReferrer = backingToken.balanceOf(platformReferrer) - initialBalances.platformReferrer;
        deltas.tradeReferrer = backingToken.balanceOf(tradeReferrer) - initialBalances.tradeReferrer;
        deltas.protocol = backingToken.balanceOf(contentCoin.protocolRewardRecipient()) - initialBalances.protocol;
        deltas.doppler = backingToken.balanceOf(contentCoin.dopplerFeeRecipient()) - initialBalances.doppler;
    }
    ...
    function test_estimateAfterSwapCurrencyAmount() public {
        ...
        deal(address(backingToken), trader, tradeAmount * 2);
        _swapSomeCurrencyForCoin(ICoin(address(backingCreatorCoin)), address(backingToken), tradeAmount, trader);

        // Record initial balances
        RewardBalances memory initialBalances = _recordBackingBalances();
    ...
    function test_rewards_creator_referrer_only() public {
        ...
        address trader = users.buyer;
        deal(address(backingToken), trader, tradeAmount * 2); // Give extra for initial swap

        // Step 1: Swap BACKING for backing creator coin
        _swapSomeCurrencyForCoin(ICoin(address(backingCreatorCoin)), address(backingToken), tradeAmount, trader);
        ...
        RewardBalances memory rewards = _calculateBackingRewardDeltas(initialBalances);
