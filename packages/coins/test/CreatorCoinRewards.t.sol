...
contract CreatorCoinRewardsTest is BaseTest {
    ...
    // Helper function to record initial BACKING token balances for all reward recipients
    function _recordBackingBalances() internal view returns (RewardBalances memory balances) {
        return _recordBalances(backingToken);
    }
    ...
    function _recordInitialBalances() internal view returns (RewardBalances memory balances) {
        return _recordBackingBalances();
    }

    // Helper function to calculate BACKING token reward deltas after trade
    function _calculateBackingRewardDeltas(RewardBalances memory initialBalances) internal view returns (RewardBalances memory deltas) {
        return
    ...
    function _calculateRewardDeltas(RewardBalances memory initialBalances) internal view returns (RewardBalances memory deltas) {
        return _calculateBackingRewardDeltas(initialBalances);
    }
    ...
    // Helper function to sell creator coin for BACKING tokens
    ...
    function test_rewards_platform_referrer_only() public {
        ...
        // Perform trade
        uint256 backingFees = _buyCreatorCoin(tradeAmount, false);
        ...
        RewardBalances memory expected = RewardTestHelpers.calculateExpectedRewards(backingFees, true, false);
    ...
    function test_rewards_trade_referrer_only() public {
        ...
        uint256 backingFees = _buyCreatorCoin(tradeAmount, true);
        ...
        // Calculate market rewards from total fees
        // Use estimated market rewards (already accounts for LP deduction and slippage)
        RewardBalances memory expected = RewardTestHelpers.calculateExpectedRewards(backingFees, false, true);
        RewardTestHelpers.assertRewardsApproxEqRel(rewards, expected);
    }
    ...
    function test_rewards_both_referrers() public {
        ...
        // Calculate market rewards from total fees
        // Use estimated market rewards (already accounts for LP deduction and slippage)
        RewardBalances memory expected = RewardTestHelpers.calculateExpectedRewards(backingFees, true, true);
        RewardTestHelpers.assertRewardsApproxEqRel(rewards, expected);
    }
    ...
    function test_rewards_no_referrers() public {
        ...
        // Calculate market rewards from total fees
        // Use estimated market rewards (already accounts for LP deduction and slippage)
        RewardBalances memory expected = RewardTestHelpers.calculateExpectedRewards(backingFees, false, false);
        RewardTestHelpers.assertRewardsApproxEqRel(rewards, expected);
    }
    ...
    function test_buy_then_sell_both_referrers() public {
        ...
        // Step 1: Buy creator coin (BACKING -> Creator Coin)
        ...
        // Record initial balances for both BACKING and creator coin
        RewardBalances memory initialBackingBalances = _recordBackingBalances();

        // Step 2: Sell creator coin (Creator Coin -> BACKING)
        uint128 sellAmount = uint128(creatorCoinBalance);
        uint256 sellBackingFees = _sellCreatorCoin(sellAmount, true);

        // Calculate final reward deltas for both currencies
        RewardBalances memory finalBackingRewards = _calculateBackingRewardDeltas(initialBackingBalances);

        // Calculate total market rewards from both trades
        RewardBalances memory expectedTotalRewards = RewardTestHelpers.calculateExpectedRewards(sellBackingFees, true, true);

        // Validate BACKING token distributions (where all final rewards end up)
        RewardTestHelpers.assertRewardsApproxEqRel(finalBackingRewards, expectedTotalRewards);
    }
    ...
    function test_estimateAfterSwapCurrencyAmount() public {
        ...
        RewardBalances memory finalRewards = _calculateBackingRewardDeltas(initialBalances);
