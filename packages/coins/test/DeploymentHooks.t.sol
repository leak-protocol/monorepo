...
contract DeploymentsHooksTest is BaseTest {
    ...
    function test_buySupplyWithEthUsingV4Hook_withExactInputMultiHop(uint256 initialOrderSize) public {
        ...
        // lets try weth to usdc to backing
        ...
        bytes memory hookData = _encodeExactInput(
            ...
            ISwapRouter.ExactInputParams({
                path: abi.encodePacked(address(weth), poolFee, USDC_ADDRESS, poolFee, backing),
        ...
        assertEq(coinV4.currency(), backing, "currency");
        ...
        // assertGt(IERC20(backing).balanceOf(address(pool)), 0, "Pool BACKING balance");
    }
    ...
    function test_buySupplyWithEthUsingV3Hook_revertsWhenBadCall() public {
        ...
        factory.deployWithHook{value: 0.0001 ether}(
            ...
            "Testcoin",
            "TEST",
            _generateDefaultPoolConfig(backing),
            users.platformReferrer,
            address(buySupplyWithSwapRouterHook),
            hookData
        );
    ...
    function test_invalidHookReverts() public {
        ...
        vm.expectRevert(ILeakFactory.InvalidHook.selector);
        vm.prank(users.creator);
        _deployWithHook(address(fakeHook), hookData, backing);
    }

    function test_noHookWorksAsNormal() public {
        // Expect the transaction to revert with InvalidHook error
        vm.prank(users.creator);
        _deployWithHook(address(0), bytes(""), backing);
    }
    ...
    function test_creatorCoin_deployWithHook_buySupplyWithEth() public {
        ...
        assertEq(creatorCoin.currency(), backing, "currency should be BACKING");
