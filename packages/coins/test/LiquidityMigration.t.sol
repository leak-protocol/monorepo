...
contract LiquidityMigrationTest is BaseTest {
    ...
    function test_migrateLiquidity_revertsSwapsOnOldPoolKey() public {
        ...
        ILeakCoinHook.PoolCoin memory poolCoinBefore = hook.getPoolCoin(poolKey);
        assertEq(poolCoinBefore.coin, address(coinV4), "pool coin should exist before migration");

        ILeakCoinHook.PoolCoin memory poolCoinByHashBefore = hook.getPoolCoinByHash(poolKeyHash);
        ...
        ILeakCoinHook.PoolCoin memory poolCoinAfter = hook.getPoolCoin(poolKey);
        ...
        ILeakCoinHook.PoolCoin memory poolCoinByHashAfter = hook.getPoolCoinByHash(poolKeyHash);
        ...
        vm.expectRevert(abi.encodeWithSelector(ILeakCoinHook.NoCoinForHook.selector, poolKey));
    ...
    function test_migrateLiquidity_revertsIfNotCalledByCoin() public {
        ...
        vm.prank(users.creator);
        vm.expectRevert(abi.encodeWithSelector(ILeakCoinHook.OnlyCoin.selector, users.creator, address(coinV4)));
    ...
    //     bytes memory creationCode = HooksDeployment.makeHookCreationCode(address(poolManager), coinVersionLookup, trustedMsgSenderLookup, upgradeGate, address(mockLeakLimitOrderBook));
