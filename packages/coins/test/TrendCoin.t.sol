...
contract TrendCoinTest is BaseTest {
    ...
    function test_deployTrendCoin_emitsEventWithPoolConfig() public {
        ...
        emit ILeakFactory.TrendCoinCreated(
            address(this),
            symbol,
            address(0),
    ...
    function _recordBackingBalances() internal view returns (RewardBalances memory balances) {
        ...
        balances.protocol = backingToken.balanceOf(trendCoin.protocolRewardRecipient());
        balances.doppler = backingToken.balanceOf(trendCoin.dopplerFeeRecipient());
    }

    function _buyTrendCoin(uint128 amountIn) internal returns (uint256 feeCurrency) {
        deal(address(backingToken), users.buyer, amountIn);

        vm.warp(block.timestamp + 1 days);

        vm.startPrank(users.buyer);
        UniV4SwapHelper.approveTokenWithPermit2(permit2, address(router), address(backingToken), uint128(amountIn), uint48(block.timestamp + 1 days));
    ...
    function test_trendCoin_feeDistribution_100PercentToProtocol() public {
        ...
        uint128 tradeAmount = 1000 ether; // 1000 BACKING tokens

        // Record initial protocol balance
        address protocolRecipient = trendCoin.protocolRewardRecipient();
        uint256 initialProtocolBalance = backingToken.balanceOf(protocolRecipient);
        ...
        uint256 finalProtocolBalance = backingToken.balanceOf(protocolRecipient);
        ...
        // - Total fees = tradeAmount * 1% = 10 BACKING
        // - Protocol receives = 10 BACKING (100% of fees, no LP remint for TrendCoins)
        ...
        // Verify actual value is reasonable (should be ~10 BACKING for 1000 BACKING trade at 1% fee)
        assertGt(protocolReward, 9.9 ether, "Protocol reward should be > 9.9 BACKING");
        assertLt(protocolReward, 10.1 ether, "Protocol reward should be < 10.1 BACKING");
    ...
    function test_trendCoin_minimalFeeAfterLaunchDuration() public {
        ...
        deal(address(backingToken), trader, amountIn);
        vm.startPrank(trader);
        UniV4SwapHelper.approveTokenWithPermit2(permit2, address(router), address(backingToken), amountIn, uint48(block.timestamp + 1 days));
        (bytes memory commands, bytes[] memory inputs) = UniV4SwapHelper.buildExactInputSingleSwapCommand(
            address(backingToken),
            amountIn,
            address(trendCoin),
    ...
    function test_trendCoin_curveConfiguration() public {
        ...
        // Verify currency is BACKING
        assertEq(currency, CoinConstants.CREATOR_COIN_CURRENCY, "Currency should be BACKING");
    ...
    function test_revertWhen_deployTrendCoin_ethWithoutHook() public {
        ...
        vm.expectRevert(ILeakFactory.EthTransferInvalid.selector);
    ...
    function test_setTrendCoinPoolConfig_ownerCanSet() public {
        ...
        address factoryOwner = LeakFactoryImpl(address(factory)).owner();
    ...
    function test_setTrendCoinPoolConfig_emitsEvent() public {
        ...
        emit ILeakFactory.TrendCoinPoolConfigUpdated(expectedPoolConfig);
    ...
    function test_revertWhen_setTrendCoinPoolConfig_emptyArrays() public {
        ...
        vm.prank(factoryOwner);
        vm.expectRevert(ILeakFactory.InvalidConfig.selector);
        factory.setTrendCoinPoolConfig(currency, tickLower, tickUpper, numDiscoveryPositions, maxDiscoverySupplyShare);
    }
    ...
    function test_revertWhen_deployTrendCoin_configNotSet() public {
        ...
        LeakFactory newFactoryProxy = new LeakFactory(proxyShim);

        // Create a new hook registry that includes the new factory as an owner
        LeakHookRegistry newHookRegistry = new LeakHookRegistry();
        ...
        LeakFactoryImpl newFactoryImpl = new LeakFactoryImpl(
            address(coinV4Impl),
        ...
        UUPSUpgradeable(address(newFactoryProxy)).upgradeToAndCall(
            ...
            abi.encodeWithSelector(LeakFactoryImpl.initialize.selector, address(this))
        );

        ILeakFactory newFactory = ILeakFactory(address(newFactoryProxy));

        // Try to deploy trend coin without setting config first
        vm.expectRevert(ILeakFactory.TrendCoinPoolConfigNotSet.selector);
...
contract TrendCoinFeeTest is BaseTest {
    ...
    function test_trendCoin_postLaunchFeeIs1Bps() public {
        ...
        address protocolRecipient = trendCoin.protocolRewardRecipient();
        uint256 protocolBefore = backingToken.balanceOf(protocolRecipient);

        deal(address(backingToken), users.buyer, tradeAmount);
        vm.startPrank(users.buyer);
        UniV4SwapHelper.approveTokenWithPermit2(permit2, address(router), address(backingToken), tradeAmount, uint48(block.timestamp + 1 days));
        (bytes memory commands, bytes[] memory inputs) = UniV4SwapHelper.buildExactInputSingleSwapCommand(
            address(backingToken),
            tradeAmount,
            address(trendCoin),
        ...
        uint256 protocolAfter = backingToken.balanceOf(protocolRecipient);
        uint256 protocolReward = protocolAfter - protocolBefore;

        // 0.01% of 1M = 100 BACKING. Allow 10% tolerance for swap path conversion.
        ...
        // Sanity: must be far below what 1% would yield (10,000 BACKING)
