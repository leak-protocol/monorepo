...
import {LeakCoinHook} from "../../src/hooks/LeakCoinHook.sol";
...
contract V4TestSetup is Test, ContractAddresses {
    ...
    // Tokens
    IERC20Metadata internal backingToken;
    ...
    function _setUpWithBlockNumber(uint256 forkBlockNumber) internal {
        mockLeakLimitOrderBook = new MockLeakLimitOrderBook();
        _setUpWithBlockNumber(forkBlockNumber, address(mockLeakLimitOrderBook));
    }

    function _setUpWithBlockNumber(uint256 forkBlockNumber, address _limitOrderBook) internal {
        // leak.ai: fork Avalanche C-Chain instead of Base. The block number argument that
        // callers pass is a Base block and is meaningless here, so the pinned Avalanche
        // block in ContractAddresses is used for every fork test.
        forkBlockNumber;
        forkId = vm.createSelectFork("avalanche", AVALANCHE_FORK_BLOCK);

        // Doppler's Airlock has no deployment on Avalanche. BaseCoin calls owner() on it to
        // resolve dopplerFeeRecipient(), so place a mock at the address the constants expect.
        vm.etch(DOPPLER_AIRLOCK, address(new MockAirlock(address(this))).code);
        MockAirlock(DOPPLER_AIRLOCK).setOwner(makeAddr("dopplerFeeRecipient"));

        weth = IWETH(WETH_ADDRESS);
        usdc = IERC20Metadata(USDC_ADDRESS);
        backingToken = IERC20Metadata(BACKING_TOKEN_ADDRESS);
        ...
        factory = ILeakFactory(address(new LeakFactory(address(mockUpgradeableImpl))));
        ...
        factoryImpl = new LeakFactoryImpl(address(coinV4Impl), address(creatorCoinImpl), address(trendCoinImpl), address(hook), address(leakHookRegistry));
        UUPSUpgradeable(address(factory)).upgradeToAndCall(address(factoryImpl), "");
        factory = ILeakFactory(address(factory));

        LeakFactoryImpl(address(factory)).initialize(users.factoryOwner);

        // Set trend coin pool config
        _setTrendCoinPoolConfig();

        vm.label(address(factory), "BACKING_FACTORY");
        ...
        vm.label(address(backingToken), "$BACKING");
        ...
        vm.label(address(mockLeakLimitOrderBook), "LIMIT_ORDER_BOOK");
    }

    function _setUpNonForked() internal {
        mockLeakLimitOrderBook = new MockLeakLimitOrderBook();
        _setUpNonForked(address(mockLeakLimitOrderBook));
    }
    ...
    function _setUpNonForked(address limitOrderBook) internal {
        ...
        // Deploy mock BACKING token at the correct address
        deployCodeTo("SimpleERC20.sol:SimpleERC20", abi.encode("BACKING", "$BACKING"), BACKING_TOKEN_ADDRESS);
        backingToken = IERC20Metadata(BACKING_TOKEN_ADDRESS);

        // Fund the pool manager with BACKING tokens
        deal(address(backingToken), address(poolManager), 1_000_000_000e18);
    ...
    function _deployHooksNonForked(address limitOrderBook) internal {
        ...
        hook = LeakCoinHook(payable(DeployHelper.deploy(hookCreationCode, salt)));
    ...
    function _deployFeeEstimatorHook(address hooks) internal {
        ...
        deployCodeTo(
            ...
            abi.encode(address(poolManager), address(factory), newLookup, hookUpgradeGate, _getLimitOrderBookAddress(), address(leakHookRegistry)),
            hooks
        );
    }

    function _getLimitOrderBookAddress() internal view virtual returns (address) {
        return address(mockLeakLimitOrderBook);
    }
