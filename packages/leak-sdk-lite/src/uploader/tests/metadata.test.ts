...
describe("getURLFromUploadResult", () => {
  ...
  it("should throw for invalid URLs", () => {
    const result: UploadResult = {
      // ValidMetadataURI rejects this string, which is exactly what the test asserts.
      // The type error here is deliberate — do not widen the type to remove it.
      // @ts-expect-error checks runtime behaviour for a value the type forbids
      url: "invalid-url",
      size: 100,
      mimeType: "image/png",
    };
