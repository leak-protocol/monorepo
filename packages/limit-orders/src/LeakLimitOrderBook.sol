...
import {ILeakHookRegistry} from "@zoralabs/coins/src/interfaces/ILeakHookRegistry.sol";
...
contract LeakLimitOrderBook is ILeakLimitOrderBookFull, ContractVersionBase, PermittedCallers {
    IPoolManager public immutable poolManager;
    IDeployedCoinVersionLookup public immutable zoraCoinVersionLookup;
    ILeakHookRegistry public immutable leakHookRegistry;
    address public immutable weth;

    constructor(address poolManager_, address zoraCoinVersionLookup_, address leakHookRegistry_, address owner_, address weth_) PermittedCallers(owner_) {
        require(poolManager_ != address(0), AddressZero());
        require(zoraCoinVersionLookup_ != address(0), AddressZero());
        require(leakHookRegistry_ != address(0), AddressZero());
        require(weth_ != address(0), AddressZero());

        poolManager = IPoolManager(poolManager_);
        zoraCoinVersionLookup = IDeployedCoinVersionLookup(zoraCoinVersionLookup_);
        leakHookRegistry = ILeakHookRegistry(leakHookRegistry_);
        weth = weth_;

        LimitOrderStorage.layout().maxFillCount = 50;
    }

    /// @inheritdoc ILeakLimitOrderBookFull
    ...
    function fill(PoolKey calldata key, bool isCurrency0, int24 startTick, int24 endTick, uint256 maxFillCount, address fillReferral) external override {
        ...
        if (isUnlocked) {
            require(leakHookRegistry.isRegisteredHook(msg.sender), UnlockedFillNotAllowed());
        }
        ...
        ILeakLimitOrderBookFull.FillCallbackData memory fillData = _fillData(
            canonicalKey,
            isCurrency0,
            resolvedStart,
            resolvedEnd,
        ...
        // unlock and fill
        _unlock(ILeakLimitOrderBookFull.CallbackId.FILL, abi.encode(fillData));
    }
    ...
    function fill(OrderBatch[] calldata batches, address fillReferral) external override {
        ...
        for (uint256 i = 0; i < length; ++i) {
            ...
            if (batch.orderIds.length != 0) {
                ...
                _unlock(
                    ILeakLimitOrderBookFull.CallbackId.FILL,
    ...
    ) private pure returns (ILeakLimitOrderBookFull.FillCallbackData memory data) {
        data.poolKey = key;
