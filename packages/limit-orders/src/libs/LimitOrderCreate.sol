...
import {ILeakLimitOrderBookFull} from "../ILeakLimitOrderBookFull.sol";
...
library LimitOrderCreate {
    ...
    function create(
        ...
        require(maker != address(0), ILeakLimitOrderBookFull.ZeroMaker());

        (ILeakLimitOrderBookFull.CreateCallbackData memory createData, uint256 totalSize) = _prepareCreateData(key, isCurrency0, orderSizes, orderTicks, maker);
        ...
        bytes memory result = poolManager.unlock(abi.encode(ILeakLimitOrderBookFull.CallbackId.CREATE, abi.encode(createData)));
    ...
    function handleCreateCallback(LimitOrderStorage.Layout storage state, IPoolManager poolManager, bytes memory payload) internal returns (bytes memory) {
        ILeakLimitOrderBookFull.CreateCallbackData memory data = abi.decode(payload, (ILeakLimitOrderBookFull.CreateCallbackData));
    ...
    function _validateOrderInputs(uint256[] memory orderSizes, int24[] memory orderTicks, address maker) private pure returns (uint256 total) {
        ...
        require(length == orderTicks.length, ILeakLimitOrderBookFull.ArrayLengthMismatch());

        for (uint256 i; i < length; ) {
            uint256 size = orderSizes[i];
            require(size != 0, ILeakLimitOrderBookFull.ZeroOrderSize());
            total += size;
            unchecked {
                ++i;
            }
    ...
    function _pullFunds(PoolKey memory key, bool isCurrency0, uint256 total, address payer, address book) private {
        ...
        if (coin == address(0)) {
            require(msg.value == total, ILeakLimitOrderBookFull.NativeValueMismatch());
        } else {
            require(msg.value == 0, ILeakLimitOrderBookFull.NativeValueMismatch());

            uint256 beforeBalance = IERC20(coin).balanceOf(book);

            require(IERC20(coin).transferFrom(payer, book, total), ILeakLimitOrderBookFull.InsufficientTransferFunds());
            require(IERC20(coin).balanceOf(book) == beforeBalance + total, ILeakLimitOrderBookFull.InsufficientTransferFunds());
        }
    }
    ...
    function _create(
        LimitOrderStorage.Layout storage state,
        IPoolManager poolManager,
        ILeakLimitOrderBookFull.CreateCallbackData memory data
    ...
    function _createSingleOrder(
        ...
        require(liquidity != 0, ILeakLimitOrderBookFull.ZeroRealizedOrder());
    ...
    function _mintLiquidity(IPoolManager poolManager, bool isCurrency0, MintParams memory params) private returns (uint128 realizedSize, uint128 refunded) {
        ...
        require(realizedSize != 0, ILeakLimitOrderBookFull.ZeroRealizedOrder());
    ...
    function _prepareCreateData(
        ...
        int24[] memory orderTicks,
        address maker
    ) private pure returns (ILeakLimitOrderBookFull.CreateCallbackData memory data, uint256 totalSize) {
        totalSize = _validateOrderInputs(orderSizes, orderTicks, maker);
        data = ILeakLimitOrderBookFull.CreateCallbackData({key: key, isCurrency0: isCurrency0, orderSizes: orderSizes, orderTicks: orderTicks, maker: maker});
    }
