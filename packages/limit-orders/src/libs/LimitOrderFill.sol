...
library LimitOrderFill {
    ...
    function handleFillCallback(LimitOrderStorage.Layout storage state, Context memory ctx, bytes memory callbackData) internal {
        ILeakLimitOrderBookFull.FillCallbackData memory data = abi.decode(callbackData, (ILeakLimitOrderBookFull.FillCallbackData));
        executeFill(state, ctx, data);
    }

    function executeFill(LimitOrderStorage.Layout storage state, Context memory ctx, ILeakLimitOrderBookFull.FillCallbackData memory data) internal {
        ...
        if (key.tickSpacing == 0) {
            key = data.poolKey;
            if (key.tickSpacing == 0) revert ILeakLimitOrderBookFull.InvalidPoolKey();
        }
    ...
    function _fillAcrossRange(LimitOrderStorage.Layout storage state, Context memory ctx, ILeakLimitOrderBookFull.FillCallbackData memory data) private {
    ...
    function _fillOrder(
        ...
        emit ILeakLimitOrderBookFull.LimitOrderFilled(
            order.maker,
            coinIn,
