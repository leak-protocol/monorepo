...
library LimitOrderViews {
    ...
    function validateTickRange(
        ...
        if (canonicalKey.tickSpacing == 0) {
            ...
            if (canonicalKey.tickSpacing == 0) revert ILeakLimitOrderBookFull.InvalidPoolKey();
        }
    ...
    function _validateTickRange(bool isCurrency0, int24 startTick, int24 endTick) private pure {
        if (startTick < TickMath.MIN_TICK || startTick > TickMath.MAX_TICK) {
            revert ILeakLimitOrderBookFull.InvalidFillWindow(startTick, endTick, isCurrency0);
        }
        ...
        if (isCurrency0) {
            if (startTick > endTick) revert ILeakLimitOrderBookFull.InvalidFillWindow(startTick, endTick, isCurrency0);
        } else {
            if (startTick < endTick) revert ILeakLimitOrderBookFull.InvalidFillWindow(startTick, endTick, isCurrency0);
        }
    }
