...
library LimitOrderWithdraw {
    function handleWithdrawOrdersCallback(LimitOrderStorage.Layout storage state, IPoolManager poolManager, address weth, bytes memory payload) internal {
        ILeakLimitOrderBookFull.WithdrawOrdersCallbackData memory data = abi.decode(payload, (ILeakLimitOrderBookFull.WithdrawOrdersCallbackData));
    ...
    function withdrawOrders(
        ...
        require(recipient != address(0), ILeakLimitOrderBookFull.AddressZero());
        uint256 orderCount = orderIds.length;
        require(orderCount != 0, ILeakLimitOrderBookFull.InvalidOrder());
        ...
        for (uint256 i; i < orderCount; ++i) {
            ...
            require(order.maker != address(0), ILeakLimitOrderBookFull.InvalidOrder());
            require(order.maker == maker, ILeakLimitOrderBookFull.OrderNotMaker());
            require(order.status == LimitOrderTypes.OrderStatus.OPEN, ILeakLimitOrderBookFull.OrderClosed());
            ...
            if (currentCoin != coin) {
                revert ILeakLimitOrderBookFull.CoinMismatch(orderId, coin, currentCoin);
            }
        ...
        require(totalWithdrawn >= minAmountOut, ILeakLimitOrderBookFull.MinAmountNotReached(totalWithdrawn, minAmountOut));
    }
    ...
    function _cancelOrder(
        ...
        require(key.tickSpacing != 0, ILeakLimitOrderBookFull.InvalidOrder());

        // Prevent withdrawal of crossed orders - they must be filled instead
        require(!LimitOrderCommon.hasCrossed(order, LimitOrderCommon.currentPoolTick(poolManager, key)), ILeakLimitOrderBookFull.OrderFillable());
        ...
        emit ILeakLimitOrderBookFull.LimitOrderUpdated(maker, coin, order.poolKeyHash, isCurrency0, orderTick, 0, orderId, true);
    }
}
