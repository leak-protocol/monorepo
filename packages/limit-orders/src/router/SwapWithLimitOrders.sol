...
contract SwapWithLimitOrders is ContractVersionBase, Ownable2Step, IMsgSender {
    ...
    ILeakLimitOrderBookFull public immutable leakLimitOrderBook;
    ...
    /// @param leakLimitOrderBook_ The limit order book contract
    ...
    constructor(
        ...
        ILeakLimitOrderBookFull leakLimitOrderBook_,
        ISwapRouter swapRouter_,
        address permit2_,
        address owner_
    ) Ownable(owner_) {
        require(address(poolManager_) != address(0), "PoolManager cannot be zero");
        require(address(leakLimitOrderBook_) != address(0), "LeakLimitOrderBook cannot be zero");
    ...
    function _createLimitOrders(
        ...
        if (orders.sizes.length > 0 && allocated > 0) {
            ...
            if (coinAddress != address(0)) {
                IERC20(coinAddress).approve(address(leakLimitOrderBook), allocated);
            }

            // Create orders with prefunded path
            bytes32[] memory orderIds = leakLimitOrderBook.create{value: value}(targetPool, isCoinCurrency0, orders.sizes, orders.ticks, maker);
    ...
    function _transferFundsToPoolManager(address token, uint256 amount) internal {
        ...
        if (!currency.isAddressZero()) {
            ...
            require(currency.balanceOf(address(poolManager)) == beforeBalance + amount, ILeakLimitOrderBookFull.InsufficientTransferFunds());
    ...
    function _fillOrders(PoolKey memory poolKey, bool, int24 tickBeforeSwap, int24 tickAfterSwap) internal {
        ...
        leakLimitOrderBook.fill(poolKey, isCurrency0, tickBeforeSwap, tickAfterSwap, 0, address(0));
    }
