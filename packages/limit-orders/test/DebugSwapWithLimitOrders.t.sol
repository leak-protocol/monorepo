...
contract DebugSwapWithLimitOrders is Test {
    ...
    function setUp() public {
        ...
        deployCodeTo(
            ...
            abi.encode(IPoolManager(POOL_MANAGER), ILeakLimitOrderBookFull(LIMIT_ORDER_BOOK), ISwapRouter(SWAP_ROUTER), PERMIT2, OWNER),
            ZORA_ROUTER
        );

        // Etch the modified LeakLimitOrderBook (with console.log) onto the deployed LOB address
        deployCodeTo(
            "LeakLimitOrderBook.sol:LeakLimitOrderBook",
        ...
        bytes memory hookCreationCode = HooksDeployment.makeHookCreationCode(
            POOL_MANAGER,
            ZORA_COIN_VERSION_LOOKUP, // LeakFactory implements IDeployedCoinVersionLookup
