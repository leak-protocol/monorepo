...
import {ILeakLimitOrderBookFull} from "../src/ILeakLimitOrderBookFull.sol";
...
contract LimitOrderFillTest is BaseTest {
    ...
    function test_manualFillWithDisabledHookAutoFill() public {
        ...
        deal(address(backingToken), swapper, uint256(smallSwap));
        _swapSomeCurrencyForCoin(ICoin(address(creatorCoin)), address(backingToken), smallSwap, swapper);
        ...
        deal(address(backingToken), swapper, uint256(swapAmount));
        _swapSomeCurrencyForCoin(ICoin(address(creatorCoin)), address(backingToken), swapAmount, swapper);
    ...
    function test_fillRevertsOnInvalidWindowCurrency0() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(ILeakLimitOrderBookFull.InvalidFillWindow.selector, startTick, endTick, true));
    ...
    function test_fillRevertsOnInvalidWindowCurrency1() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(ILeakLimitOrderBookFull.InvalidFillWindow.selector, startTick, endTick, false));
    ...
    function test_fillBatchConsumesOrders() public {
        ...
        ILeakLimitOrderBookFull.OrderBatch[] memory batches = new ILeakLimitOrderBookFull.OrderBatch[](1);
        batches[0] = ILeakLimitOrderBookFull.OrderBatch({key: key, isCurrency0: isCurrency0, orderIds: orderIds});
    ...
    function test_contentCoinOrderPaysOutInZora() public {
        // Setup: content coin is backed by creator coin, which is backed by backingToken
        ...
        deal(address(backingToken), users.seller, initialZoraAmount);

        // Swap ZORA -> creator coin
        _swapSomeCurrencyForCoin(ICoin(address(creatorCoin)), address(backingToken), initialZoraAmount, users.seller);
        ...
        uint256 sellerZoraBefore = backingToken.balanceOf(users.seller);
        ...
        uint256 sellerZoraAfter = backingToken.balanceOf(users.seller);
    ...
    function _movePriceBeyondTicksForContentCoin(CreatedOrderLog[] memory created) internal {
        ...
        deal(address(backingToken), mover, swapAmount * 2);
        _swapSomeCurrencyForCoin(ICoin(address(creatorCoin)), address(backingToken), swapAmount, mover);
    ...
    function test_batchFill_emptyOrderIds_skips() public {
        ...
        ILeakLimitOrderBookFull.OrderBatch[] memory batches = new ILeakLimitOrderBookFull.OrderBatch[](2);
        batches[0] = ILeakLimitOrderBookFull.OrderBatch({
            key: key,
            isCurrency0: isCurrency0,
            orderIds: new bytes32[](0) // Empty array - should be skipped
        });
        batches[1] = ILeakLimitOrderBookFull.OrderBatch({
            key: key,
            isCurrency0: isCurrency0,
            orderIds: new bytes32[](0) // Empty array - should be skipped
        });
