...
interface ILeakLimitOrderBookFullFillTickRange {
...
contract SwapWithLimitOrdersTestNonForked is SwapWithLimitOrdersTestBase {
    ...
    function test_swapWithLimitOrders_hookCallsFill() public {
        // Test that hook calls leakLimitOrderBook.fill during swap
        ...
        // Expect that leakLimitOrderBook.fill is called by the hook during the swap
        ...
        vm.expectCall(address(limitOrderBook), abi.encodeWithSelector(ILeakLimitOrderBookFullFillTickRange.fill.selector));
    ...
    function test_orderFilling_invertedDirection() public {
        ...
        LimitOrderConfig memory limitOrderConfig = _prepareLimitOrderParams(users.buyer, _defaultMultiples(), _defaultPercentages());
        deal(address(backingToken), users.buyer, DEFAULT_LIMIT_ORDER_AMOUNT);
        ...
        deal(address(backingToken), users.seller, largerSwapAmount);
    ...
    function test_reverts_emptyV4Route() public {
        ...
        SwapWithLimitOrders.SwapWithLimitOrdersParams memory params = SwapWithLimitOrders.SwapWithLimitOrdersParams({
            ...
            limitOrderConfig: limitOrderConfig,
            inputCurrency: address(backingToken),
        ...
        IERC20(address(backingToken)).approve(address(swapWithLimitOrders), DEFAULT_LIMIT_ORDER_AMOUNT);
    ...
    function test_reverts_insufficientOutputAmount() public {
        ...
        address permit2 = AddressConstants.getPermit2Address();
        IERC20(address(backingToken)).approve(permit2, type(uint256).max);
        IAllowanceTransfer(permit2).approve(address(backingToken), address(swapWithLimitOrders), uint160(type(uint160).max), type(uint48).max);
        vm.stopPrank();
    ...
    function test_reverts_whenV3OutputDoesNotMatchV4Input() public {
        ...
        bytes memory v3Route = abi.encodePacked(address(backingToken), uint24(3000), address(contentCoin));
    ...
    function test_swapUsesMakerAllowanceEvenWhenCallerDiffers() public {
        ...
        uint256 buyerBalanceBefore = IERC20(address(backingToken)).balanceOf(users.buyer);

        _executeSwapWithLimitOrders(users.buyer, params);

        // Buyer (msg.sender) paid for the swap and owns the orders
        assertLt(IERC20(address(backingToken)).balanceOf(users.buyer), buyerBalanceBefore, "buyer should fund the swap");
    }
...
contract SwapWithLimitOrdersTestForked is SwapWithLimitOrdersTestBase {
    // USDC_ADDRESS and BACKING_TOKEN_ADDRESS are inherited from ContractAddresses
    ...
    function test_swapWithLimitOrders_ERC20toV3toV4_singleHop() public {
        ...
        bytes memory v3Route = _encodeV3PathSingle(USDC_ADDRESS, 3000, BACKING_TOKEN_ADDRESS);
    ...
    function test_swapWithLimitOrders_ZORAtoV4() public {
        // Give buyer ZORA tokens
        uint256 backingAmount = 10 ether;
        deal(BACKING_TOKEN_ADDRESS, users.buyer, backingAmount);
        ...
        SwapWithLimitOrders.SwapWithLimitOrdersParams memory params = SwapWithLimitOrders.SwapWithLimitOrdersParams({
            recipient: users.buyer,
            limitOrderConfig: limitOrderConfig,
            inputCurrency: BACKING_TOKEN_ADDRESS, // ERC20 input
            inputAmount: backingAmount,
        ...
        assertEq(IERC20(BACKING_TOKEN_ADDRESS).balanceOf(users.buyer), 0, "buyer should have spent all ZORA");
        assertGt(IERC20(address(creatorCoin)).balanceOf(users.buyer), 0, "buyer should have received coins");
    }
