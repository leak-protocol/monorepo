...
import {TestableLeakLimitOrderBook} from "./TestableLeakLimitOrderBook.sol";
...
contract BaseTest is V4TestSetup, IMsgSender {
    ...
    TestableLeakLimitOrderBook internal limitOrderBook = TestableLeakLimitOrderBook(payable(makeAddr("limitOrderBook")));
    ...
    function setUpNonForked() public virtual {
        ...
        // mockLimitOrderBookForHook = new MockLeakLimitOrderBookNoTransientStorage();
    ...
    function _setupLimitOrderBook() internal {
        ...
        // Deploy LeakLimitOrderBook with this contract as owner
        deployCodeTo(
            "TestableLeakLimitOrderBook.sol:TestableLeakLimitOrderBook",
            abi.encode(address(poolManager), address(factory), address(leakHookRegistry), address(this), address(weth)),
            address(limitOrderBook)
        );
        require(limitOrderBook.owner() == address(this), "LeakLimitOrderBook owner is not this contract");
        ...
        // Now create the real LeakLimitOrderBook for tests that need it
        _deployTestCoins();
    }
    ...
    function _executeSingleHopSwap(address trader, uint256 amountIn, PoolKey memory poolKey, bytes memory hookData) internal {
        ...
        address currencyOut = Currency.unwrap(poolKey.currency0) == address(backingToken)
    ...
    function _movePriceBeyondTicks(CreatedOrderLog[] memory created) internal virtual {
        ...
        if (needTickUp == sellCoinIncreaseTick) {
            ...
            _swapSomeCoinForCurrency(ICoin(address(creatorCoin)), address(backingToken), uint128(swapAmount), mover);
        } else {
            // Buy coin to move tick in required direction
            deal(address(backingToken), mover, swapAmount);
            _swapSomeCurrencyForCoin(ICoin(address(creatorCoin)), address(backingToken), uint128(swapAmount), mover);
        }
    }
